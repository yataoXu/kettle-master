...
public class TransQuartz implements Job {
		...
		if (null != DbConnectionObject && DbConnectionObject instanceof DBConnectionModel) {// 首先判断数据库连接对象是否正确
			try {// 判断转换类型
				if (null != KRepositoryObject && KRepositoryObject instanceof KRepository) {// 证明该转换是从资源库中获取到的
					addOrRemoveMonitor(DbConnectionObject, Integer.valueOf(userId), Integer.valueOf(transId));
	 ...
	 * @return void
	 */
	private void addOrRemoveMonitor(Object DbConnectionObject, Integer uId, Integer transId) throws SQLException {
		DBConnectionModel DBConnectionModel = (DBConnectionModel) DbConnectionObject;
		ConnectionSource source = ConnectionSourceHelper.getSimple(DBConnectionModel.getConnectionDriveClassName(),
				DBConnectionModel.getConnectionUrl(), DBConnectionModel.getConnectionUser(),
				DBConnectionModel.getConnectionPassword());
		...
		if (null != templateOne) {
			...
			sqlManager.insert(KTransMonitor.class, kTransMonitor, false);
		}
	}
}