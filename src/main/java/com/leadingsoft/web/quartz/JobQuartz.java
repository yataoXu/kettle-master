...
public class JobQuartz implements Job {
		...
		if (null != DbConnectionObject && DbConnectionObject instanceof DBConnectionModel) {// 首先判断数据库连接对象是否正确
			try {// 判断作业类型
				if (null != KRepositoryObject && KRepositoryObject instanceof KRepository) {// 证明该作业是从资源库中获取到的
					addOrRemoveMonitor(DbConnectionObject, Integer.valueOf(userId), Integer.valueOf(jobId));
	 ...
	 * @return void
	 */
	public void runRepositoryJob(Object KRepositoryObject, Object DbConnectionObject, String jobId, String jobPath,
			String jobName, String userId, String logLevel, String logFilePath) throws KettleException {
	...
	public void runFileJob(Object DbConnectionObject, String jobId, String jobPath, String jobName, String userId,
			String logLevel, String logFilePath) throws KettleXMLException {
	 ...
	 * @return void
	 */
	private void addMonitor(Object DbConnectionObject, KJobRecord kJobRecord) throws IOException, SQLException {
		// 写入转换运行记录到数据库
		DBConnectionModel DBConnectionModel = (DBConnectionModel) DbConnectionObject;
		ConnectionSource source = ConnectionSourceHelper.getSimple(DBConnectionModel.getConnectionDriveClassName(),
				DBConnectionModel.getConnectionUrl(), DBConnectionModel.getConnectionUser(),
		...
		if (null != templateOne) {
			...
			kJobMonitor.setMonitorJob(kJobRecord.getRecordJob());
			kJobMonitor.setAddUser(kJobRecord.getAddUser());
			...
			sqlManager.insert(KJobMonitor.class, kJobMonitor, false);
		}
		DSTransactionManager.commit();
	}

	private void addOrRemoveMonitor(Object DbConnectionObject, Integer uId, Integer jobId) throws SQLException {
		DBConnectionModel DBConnectionModel = (DBConnectionModel) DbConnectionObject;
		ConnectionSource source = ConnectionSourceHelper.getSimple(DBConnectionModel.getConnectionDriveClassName(),
				DBConnectionModel.getConnectionUrl(), DBConnectionModel.getConnectionUser(),
				DBConnectionModel.getConnectionPassword());
		...
		if (null != templateOne) {
			StringBuilder runStatusBuilder = new StringBuilder();
			if (templateOne.getMonitorStatus() == 1) {
			...
			kJobMonitor.setAddUser(uId);
			kJobMonitor.setMonitorSuccess(0);
			kJobMonitor.setMonitorFail(0);
			StringBuilder runStatusBuilder = new StringBuilder();
			// runStatusBuilder.append(new Date().getTime());
			runStatusBuilder.append(new Date().getTime()).append(Constant.RUNSTATUS_SEPARATE);
			kJobMonitor.setRunStatus(runStatusBuilder.toString());
			kJobMonitor.setMonitorStatus(1);
			sqlManager.insert(KJobMonitor.class, kJobMonitor, false);
